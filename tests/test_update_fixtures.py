...
# Live HV week-2 semi card: round header PLUS numbered inner subtype.
SEMI_FINAL_1 = _match_card(
    "<b>Semi Finals</b><div><b>Semi Final 1</b></div>",
    "Sun 06 Sep 2026 15:00",
    "State Sport Centres, Parkville",
    "https://www.hockeyvictoria.org.au/venues/25879/15222",
    "Playing",
    "Womens Premier League Reserves - 2026 Waverley Hockey Club",
    "https://www.hockeyvictoria.org.au/games/team/25879/412510",
)

# Live HV week-2 prelim card: round header with no inner subtype.
PRELIMINARY_FINALS = _match_card(
    "<b>Preliminary Finals</b>",
    "Sun 06 Sep 2026 15:00",
    "Monash University",
    "https://www.hockeyvictoria.org.au/venues/25879/16417",
    "Playing",
    "Mens Metro 1 South - 2026 Collegians-X Hockey Club",
    "https://www.hockeyvictoria.org.au/games/team/25879/412260",
)
...
class ParseFinalsTests(unittest.TestCase):
    ...
    def test_successive_finals_are_not_collapsed(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ("2026-09-13", "Camberwell Hockey Club"),
            ],
        )
    ...
    def test_semi_finals_header_and_semi_final_1_are_one_fixture(self):
        soup = BeautifulSoup(SEMI_FINAL_1, "html.parser")
        fixtures = parse_fixtures_from_soup(soup)
        self.assertEqual(len(fixtures), 1)
        semi = fixtures[0]
        self.assertEqual(semi["date"], "2026-09-06")
        self.assertEqual(semi["time"], "3:00pm")
        self.assertEqual(semi["opponent"], "Waverley Hockey Club")
        self.assertEqual(semi["venue"], "Away")
        self.assertIsNone(semi["result"])

    def test_preliminary_finals_header_only_card_is_kept(self):
        soup = BeautifulSoup(PRELIMINARY_FINALS, "html.parser")
        fixtures = parse_fixtures_from_soup(soup)
        self.assertEqual(len(fixtures), 1)
        prelim = fixtures[0]
        self.assertEqual(prelim["date"], "2026-09-06")
        self.assertEqual(prelim["time"], "3:00pm")
        self.assertEqual(prelim["opponent"], "Collegians-X Hockey Club")
        self.assertEqual(prelim["venue"], "Away")
        self.assertIsNone(prelim["result"])

    def test_elimination_then_preliminary_finals_are_both_kept(self):
        soup = BeautifulSoup(ELIMINATION_FINAL + PRELIMINARY_FINALS, "html.parser")
        fixtures = parse_fixtures_from_soup(soup)
        self.assertEqual(
            [(f["date"], f["opponent"]) for f in fixtures],
            [
                ("2026-08-30", "Maccabi Hockey Club"),
                ("2026-09-06", "Collegians-X Hockey Club"),
            ],
        )
